...
const ContactList = ({contacts ,isChannel = false}) => {
        ...
        const handleClick = (contact) =>{
                ...
                if(selectedChatData && selectedChatData._id !== contact._id){
                                ...
                                // clearing previus messages which are not realted to current contact
                }
        };
                                ...
                                <div className="flex gap-5 items-center justify-start text-neutral-300">
                                        ...
                                        <Avatar className = "h-10 w-10 rounded-full overflow-hidden">
                                                { contact.image ? <AvatarImage src={`${HOST}/${contact.image}`} alt = 'profile' className = 'object-cover w-full h-full bg-black' /> : (
                                                        <div className = {`${(selectedChatData && selectedChatData._id === contact._id) ? "bg-[#ffffff22] border border-white/70": getColor(contact.color)} h-10 w-10 text-lg border-[1px] flex items-center justify-center rounded-full `}  > 
